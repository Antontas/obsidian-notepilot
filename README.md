...
- **登录系统**：阿里云百炼凭证登录，登录时真实校验 API Key，登录态持久化，支持退出登录
...
- **划词提问**：编辑器选中文本 → 选区旁自动浮出「问问 AI」按钮（也可用右键菜单或命令「划词提问」）→ 选中文本自动引用到输入框，直接提问
...
6. 划词提问：编辑器选中文本 → 选区旁自动浮出「问问 AI」按钮（或右键菜单/命令面板「划词提问」）→ 在面板输入问题发送
