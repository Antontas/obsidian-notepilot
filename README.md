...
2. 在登录页下拉选择服务商（自动填充 Base URL 与默认模型），输入对应 API Key，点击登录；Ollama 本地服务与自定义服务商可无需密钥
   - 「自定义服务商（OpenAI 兼容）」：任意第三方中转/自建服务，手动填写 Base URL 与模型名
   - 「Anthropic Claude（原生协议）」：直连 Anthropic Messages API（`x-api-key` 认证）
   - 「Google Gemini（原生协议）」：直连 Google Generative Language API（`?key=` 认证）
